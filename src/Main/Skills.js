...
export default function Skills(props) {
  ...
  const transitions = useTransition(open ?props.data.skillset : [], item => item.name, {
    ref: transRef,
    unique: true,
    trail: 400 / props.data.skillset.length,
  ...
  useChain(open ? [springRef, transRef] : [transRef, springRef], [0, open ? 0.1 : 0.6])
    ...
    return (
        <Container style={{ ...rest }} onClick={() => set(!open)} isOpen>            
